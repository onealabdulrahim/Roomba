...
def Control(X,Y,Um,XFinal,YFinal):
	...
	Y_d = (a*(3*X + 5/2))/(math.pow((math.pow(((3*math.pow(X,2))/2 + (5*X)/2),2) + 1) , (1/2))) - (3*math.pow(X,2))/2 - (5*X)/2 - ((a*(3*X + 5/2)*(math.pow(((3*math.pow(X,2))/2 + (5*X)/2),2))))/(math.pow((math.pow(((3*math.pow(X,2))/2 + (5*X)/2),2) + 1),(3/2)))
	theta_d = math.atan2(Y_d,X_d) + eps*math.copysign(1,(Y - (-0.5*math.pow(X,3) - 1.25*math.pow(X,2))))
