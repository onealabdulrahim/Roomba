...
def Control(X, Y, Um, XFinal, YFinal):
    norm = math.pow(math.pow((X - XFinal), 2) +
                    math.pow((Y - YFinal), 2), (1 / 2))
    Vd = 0.1 / (1 + 1 / (3000 * math.pow(norm, 2)))
    a = 0.1
    fprime = -1.5 * math.pow(X, 2) - 2.5 * X
    eps = 0.5
    X_d = (a * (3 * X + 5 / 2) * ((3 * math.pow(X, 2)) / 2 + (5 * X) / 2)) / \
        (math.pow((math.pow(((3 * math.pow(X, 2)) / 2 + (5 * X) / 2), 2) + 1), (3 / 2))) + 1
    Y_d = (a * (3 * X + 5 / 2)) / (math.pow((math.pow(((3 * math.pow(X, 2)) / 2 + (5 * X) / 2), 2) + 1), (1 / 2))) - (3 * math.pow(X, 2)) / 2 - (5 * X) / 2 - \
        ((a * (3 * X + 5 / 2) * (math.pow(((3 * math.pow(X, 2)) / 2 + (5 * X) / 2), 2)))) / \
